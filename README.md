...
В проекте реализована непрерывная выборка сигнала с одноканального ADC, после чего результаты преобразования отправляются на компьютер через интерфейс UART (адаптер CH340G). Значения выводятся в текстовой форме с временной паузой, что обеспечивает стабильный и наглядный мониторинг.
